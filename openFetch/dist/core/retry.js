...
import { ensureIdempotencyKeyHeader, generateIdempotencyKey, hasIdempotencyKeyHeader, } from "../helpers/idempotencyKey.js";
...
async function sleepBackoff(ms, ctx) {
...
function applyPostIdempotencyKey(request, ro) {
    if (ro.maxAttempts <= 1)
        return;
    if (!ro.autoIdempotencyKey)
        return;
    ...
    if (m !== "POST")
        return;
    if (hasIdempotencyKeyHeader(request.headers))
        return;
...
function resolveRetryRequestUrl(ctx) {
    const u = ctx.request.url;
    if (u === undefined || u === "")
        return "";
    try {
        return buildURL(u, ctx.request);
    }
    catch {
        return String(u);
    }
 ...
 */
function throwIfExternalAborted(ctx) {
    const sig = ctx.request.signal;
    if (sig == null || typeof sig !== "object" || !sig.aborted)
        return;
    const err = new OpenFetchError("Request aborted", {
        config: ctx.request,
        code: "ERR_CANCELED",
...
function throwRetryDeadlineExceeded(ctx) {
...
function assertWithinRetryDeadline(ctx, deadlineMono) {
...
export function createRetryMiddleware(factoryDefaults) {
    return async (ctx, next) => {
        ...
        let attempt = 0;
        const deadlineMono = ro.timeoutTotalMs != null && ro.timeoutTotalMs > 0
            ? monotonicNowMs() + ro.timeoutTotalMs
            : null;
        const enforceTotalAbort = deadlineMono != null && ro.enforceTotalTimeout;
        while (true) {
            attempt += 1;
